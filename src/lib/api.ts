...
export const api = {
  ...
  // ─── Google review rewards (config getters live above) ───────
  reviewStats: () => req<{ total: number; pending: number; issued: number; requests_30d: number; next_reward_at: string | null; next_reward_name: string | null }>('/api/reviews/stats'),

  // ─── Push reach (how many customers can actually be reached) ──
  pushReach: () => req<{ total: number; reachable: number; percent: number }>('/api/push/reach'),
...
export interface TapCollectResult { success: boolean; customer_id: string; customer_name: string; user_id: string | null; unique_key: string | null; business_name: string; card_name: string | null; card_color: string | null; card_type: string; reward_desc: string | null; points_earned: number | null; total_points: number | null; prev_stamps: number | null; new_stamps: number | null; goal_stamps: number | null; rewards_earned: number | null; reward_ready: boolean; welcome_coupon: { title: string; barcode: string; expires_at: string } | null; next_visit_free: boolean; tap_promo: string | null; review: { url: string; reward_label: string; reward_type: 'stamp' | 'coupon'; stamp_count: number; days_to_wait: number } | null; }
